...
documentation, workflows, status checks, a readme file, right?  I can empathize with that!! 
...
**JavaScript** and **TypeScript** and is the predecessor to **NodeJS**.  It's built by the 
same person who created **NodeJS**, <URL link="https://en.wikipedia.org/wiki/Ryan_Dahl" 
text="Ryan Dayl"/>. His experience and past **NodeJS** mistakes have helped drive the design 
...
- Improve pull requests status checks
...
## <span class="color-section">So how do you do use Deno anyway?</span>

There are different ways to go about it depending on your needs.  You can host the **Deno** 
scripts alongside your project or in a central location.  I have done both with no problems
and great success.

I currently centralize all of my organization's CICD needs like **Deno** scripts and reusable workflows in a 
single repository named _Infrastructure_. Having a centralized repo is also 
nice because I can version my scripts so I can methodically roll out changes to the consuming
projects when I need to.
...
The **Deno** eco-system has its own runtime API for various tasks just like **NodeJS**  does.  
One of those tasks is being able to get the arguments that you can pass to any **Deno** script
using `Deno.args`.

In the code, we added a check to verify whether or not a **single** argument was passed in.  
If zero or more than one arguments have been passed in, the script will exit with an error code of 1.  
...
If these folders don't already exist, create one with the name _**.github**_ in the root of the repository 
and _**workflows**_ inside the _**.github**_ folder.
...
The first step will be the checkout action which is required in order to get access the script file
in the repository.  
...
after the script file path are treated as arguments.
...
permission switches/options because we aren't doing anything that requires permission. If you try to execute a script 
without explicitly allowing access to certain operations, you will get an error
...
do simple tasks in a project such as cleanup, or in this case, for **GitHub** workflows to help automate
